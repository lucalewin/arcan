...
pub async fn handle_vault_create(
    ...
    let encrypted_vsk_payload = encrypt_payload(kek, &vsk, &vault_id)?;
    let packed_encrypted_vsk = encrypted_vsk_payload.pack();

    // 3. Encrypt the Vault Name with the VSK (AAD = vault_id)
    let encrypted_name_payload = encrypt_payload(&vsk, name.as_bytes(), &vault_id)?;
    let packed_encrypted_name = encrypted_name_payload.pack();
...
pub async fn handle_vault_list(
    ...
    for vault in vaults {
        ...
        let encrypted_vsk_payload = EncryptedPayload::unpack(&vault.encrypted_vsk)?;
        let vsk_bytes = decrypt_payload(kek, &encrypted_vsk_payload, &vault.id)
        ...
        let encrypted_name_payload = EncryptedPayload::unpack(&vault.encrypted_name)?;
        let name_bytes = decrypt_payload(&vsk, &encrypted_name_payload, &vault.id)
...
pub async fn handle_vault_delete(
    ...
    let result = sqlx::query!(
        "UPDATE vaults SET is_deleted = 1, is_dirty = 1, updated_at = ?1 WHERE id = ?2",
        now,
        vault_id
    )
    .execute(pool)
